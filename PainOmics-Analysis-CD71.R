...
rownames(sampleTablelong) <- sampleTablelong$sampleName
rownames(sampleTablelong) <- gsub("_counts.txt", "",rownames(sampleTablelong))
colnames(snm_countsorig) <- gsub("_counts.txt", "",colnames(snm_countsorig))
p <- pca(snm_countsorig, metadata = sampleTablelong, removeVar = 0.1)
...
biplot(p, lab=NULL, colby = c('Status'), 
       ...
       legendPosition="right")+ geom_point()  + theme_minimal() +
  ggforce::geom_mark_ellipse(aes(fill = sampleTablelong$Status, color = sampleTablelong$Status)) +
